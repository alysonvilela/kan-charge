          ...
          <ScrollArea className="h-40">
            ...
            <ul className="space-y-2 pr-3">
              {filesToUpload.map((fileUploadProgress) => {
                return (
                  <li key={fileUploadProgress.File.lastModified} className="border rounded-lg">
                    <div className="flex items-start justify-between p-4">
                      <div className="space-y-2">
                        <TableIcon />
                        <h4 className="text-gray-800 font-semibold">{getFileIconAndColor(fileUploadProgress.File).icon}</h4>
                        <p className="text-gray-600 text-sm">{fileUploadProgress.File.name.slice(0, 25)} - {fileUploadProgress.progress}%</p>
                      </div>
                      <button
                        onClick={() => {
                        ...
                        className="text-gray-700 text-sm border rounded-lg px-3 py-2 duration-150 hover:bg-gray-100">Cancelar</button>
                    </div>
                  </li>
                );
              })}
              ...
              return (
                <li key={file.name} className="border rounded-lg unstyled">
                  <div className="flex items-start justify-between p-4">
                    <div className="space-y-2">
                      <TableIcon />
                      <h4 className="text-gray-800 font-semibold">{file.name}</h4>
